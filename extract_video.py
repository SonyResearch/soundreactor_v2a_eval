    ...
    for root, dirs, files in os.walk(video_path):
        for fname in files:
            if fname.lower().endswith('.mp4'):
                video_paths.append(Path(root) / fname)
    log.info(f'{len(video_paths)} videos found (recursively under {video_path})')

    dataset = VideoDataset(video_paths, start_time=start_time, duration_sec=audio_length)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--syncformer_ckpt_path', type=Path, required=True)
